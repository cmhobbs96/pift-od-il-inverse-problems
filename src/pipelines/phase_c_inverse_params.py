"""Phase C: Inverse parameter identification (Example 3a, Fig. 3).

Jointly infer D and κ of the nonlinear PDE  D·φ'' - κ·φ³ = f  via nested
SGLD with Jeffrey's priors on the log-transformed parameters.
"""
...
from utils.plotting import plot_figure3
from .common import emit, select_device

jax.config.update("jax_enable_x64", True)

DEFAULT_PHASE_C_CONFIG: dict[str, float | int] = {
    "seed": 42,
    "K": 20,
    "D_true": 0.1,
    "kappa_true": 1.0,
    ...
    "lambda0_log_kappa": 0.5,    # kappa_init ≈ 1.65
    # Nested SGLD
    "warmup_steps": 50_000,
    "outer_steps": 10_000,
...
def _build_grad_fns(field, source_fn, beta, n_quad, x_obs, y_obs, noise_std):
    """Build prior and posterior gradient closures for nested SGLD.

    Both follow the contract ``(phi, lam, key) -> (grad_phi, grad_lam, metrics)``.
    """

    def _physics_energy(phi, lam, x_quad):
        """β · U[φ; D, κ] differentiable in both phi and lam."""
        D = jnp.exp(lam[0])
        kappa = jnp.exp(lam[1])
    ...
    def _data_nll(phi):
        """0.5/σ² · Σ(pred - y)²  differentiable in phi."""
    ...
    def posterior_grad_fn(phi, lam, key):
        # Physics gradients (same as prior)
        key, qk = jax.random.split(key)
        x_quad = jax.random.uniform(qk, shape=(n_quad,), minval=0.0, maxval=1.0)
        ...
        # Data likelihood gradient (only phi, not lam)
...
def run_phase_c_inverse_params(
    cfg: dict | None = None,
    ...
    """Run Example 3a: jointly infer D and κ via nested SGLD.

    Returns dict with ``status``, ``lambda_chain``, ``summary``,
    ``x_grid``, ``phi_truth``, ``artifacts``, etc.
    """
    started_at = time.monotonic()
    config = dict(DEFAULT_PHASE_C_CONFIG)
    if cfg:
        config.update(cfg)
    ...
    print(
        f"[phase_c] Inverse params: D_true={D_true} kappa_true={kappa_true} "
        f"beta={beta:.0e} outer_steps={outer_steps} warmup={warmup_steps}"
    )

    emit(progress_callback, started_at, 2.0, "setup", "Initializing inverse problem")
    ...
    x_ref, phi_ref = solve_nonlinear_dirichlet_fd(
        _source_fn_np, D=D_true, kappa=kappa_true, n_points=500, bc=(0.0, 0.0),
    )
    emit(progress_callback, started_at, 5.0, "setup", "Computed FD reference solution")

    # ----- Generate observations -----
    key = jax.random.PRNGKey(int(config["seed"]))
    x_obs_np = np.linspace(0.0, 1.0, n_obs + 2)[1:-1]  # equidistant, excluding boundaries
    ...
    # Since BCs are (0,0), the ramp is zero and window = (1-x)*x.
    # Fit psi(x; theta) = phi_ref / ((1-x)*x) on interior points.
    ...
    # ----- Initialize lambda0 -----
    lambda0 = jnp.array(
        [float(config["lambda0_log_D"]), float(config["lambda0_log_kappa"])],
        dtype=jnp.float64,
    )
    emit(progress_callback, started_at, 10.0, "setup", f"Initial λ = {np.asarray(lambda0)}")

    # ----- Build gradient closures -----
    prior_grad_fn, posterior_grad_fn = _build_grad_fns(
        field, _source_fn, beta, n_quad, x_obs_jnp, y_obs_jnp, noise_std,
    )

    # ----- Preconditioner for field -----
    ...
    if result.meta.get("stopped"):
        print(
            f"[phase_c] WARNING: stopped early at step {result.meta['stop_step']}: "
    ...
    print(f"[phase_c] Posterior D: {D_mean:.4f} ± {D_std:.4f} (true={D_true})")
    print(f"[phase_c] Posterior κ: {kappa_mean:.4f} ± {kappa_std:.4f} (true={kappa_true})")
    ...
    if save_outputs:
        # Figure 3 composite
        fig3_path = str(out_fig / "phase_c_figure3.png")
        plot_figure3(
        ...
        artifacts["figure3"] = fig3_path

        # Lambda chain
        chain_path = str(out_traces / "phase_c_lambda_chain.npz")
        ...
        summary_path = str(out_tables / "phase_c_inverse_params_summary.json")
    ...
    emit(progress_callback, started_at, 100.0, "done", "Inverse parameter identification complete")
    ...
    return {
        ...
        "artifacts": artifacts,
        "config": config,
        "summary": summary if save_outputs else None,
        "runtime_sec": time.monotonic() - started_at,
    }
